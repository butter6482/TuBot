...
export const ChatWindow = ({ bot, onClose, onDelete, onUpdate }: ChatWindowProps) => {
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', text: `Hi! I'm ${bot.name}. How can I help you today?`, sender: 'bot', timestamp: new Date() },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : 'Failed to reach the AI service';
      setMessages(prev => [...prev, { id: (Date.now() + 1).toString(), text: msg, sender: 'bot', timestamp: new Date() }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-4xl h-[80vh] bg-gray-900 rounded-lg border border-gray-800 flex flex-col overflow-hidden" style={{ boxShadow: '0 0 20px rgba(0, 255, 255, 0.2)' }}>
      {/* Header with model selector */}
      <div className="p-4 flex items-center justify-between border-b border-gray-800" style={{ background: 'linear-gradient(to right, rgba(10,20,40,0.9), rgba(20,30,60,0.9))', boxShadow: '0 2px 10px rgba(0,0,0,0.2)' }}>
