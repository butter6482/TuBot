...
export function App() {
  ...
  // ✅ Se usa desde el modal vía onSave
  const createBot = (bot: Omit<Bot, 'id' | 'color'>) => {
    ...
    setBots(prev => [...prev, mapped]);       // ⬅️ agrega (no reemplaza)
    setSelectedBot(mapped);
    setIsCreatingBot(false);
  };

  const deleteBot = (id: string) => {
    setBots(prev => prev.filter(b => b.id !== id)); // ⬅️ borra solo el seleccionado
  ...
  if (authState === 'login') {
    return (
      <AuthLayout onBack={() => setAuthState('landing')}>
  ...
  return (
    <div className="w-full min-h-screen bg-black bg-opacity-95 text-white overflow-hidden flex flex-col items-center justify-center relative">
      ...
      <div className="z-10 w-full h-full flex flex-col items-center justify-center p-4 pt-16">
        {selectedBot ? (
          <ChatWindow bot={selectedBot} onClose={() => setSelectedBot(null)} onDelete={deleteBot} onUpdate={updateBot} />
        ) : (
          <>
            <TuBotLogo />
            <div className="relative w-full max-w-4xl h-[400px] my-8">
              {bots.length > 0 ? (
                <div className="absolute inset-0 flex items-center justify-center">
                  {bots.map((bot, index) => (
                    <BotSphere
                      ...
                      bot={bot}
                      index={index}
                      total={Math.min(bots.length, MAX_BOTS)}
                ...
                <div className="absolute inset-0 flex items-center justify-center flex-col space-y-6">
                  <p className="text-cyan-300 text-xl">You don’t have any bots yet</p>
            ...
            {bots.length > 0 && (
              <div className="mt-4">
                {maxBotsReached ? (
                  <p className="text-amber-400 text-center mb-2">You’ve reached the maximum of 8 bots</p>
                ) : (
                  <CreateBotButton onClick={() => setIsCreatingBot(true)} />
                )}
              </div>
      ...
      {isCreatingBot && !maxBotsReached && (

        <BotCreationModal onClose={() => setIsCreatingBot(false)} onSave={createBot} />
      )}
    </div>
  );
}
